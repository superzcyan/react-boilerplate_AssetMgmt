...
import { loadModelItems, loadAllStoredModelItems, updateSuccess, updateFailed, addSuccess, addFailed } from './actions';
import { GET_MODELITEMS, 
  ...
  export function* getModelItemsList() {
    try{
      ...
      const modelItems = yield call(getModelItems, modelTitle, currentPage,token);
      yield put(loadModelItems(modelItems.data.list, modelItems.data.list, modelItems.data.totalPage, ));
    }catch(error){
      console.log(error)
    }
  }
...
export default function* assetManufacturersSaga() {
